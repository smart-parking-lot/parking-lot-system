...
img = cv2.imread("image18.jpg", cv2.IMREAD_COLOR)#修改成要測試的圖片
gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
gray = cv2.bilateralFilter(gray, 11, 17, 17) #blur to reduce Noise
text = pytesseract.image_to_string(gray, config='-l eng --oem 3 --psm 12')  #Using oem and psm in Tesseract Raspberry Pi for better results
